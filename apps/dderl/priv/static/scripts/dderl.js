...
/*function add_to_tree(idx, parts, root)
{
    var found = false;
    ...
    if(parts[idx] == root.data) {
        add_to_tree(idx+1, parts, root)
    } else {
        found = true;
        for (var i = 0; root.children.length; ++i) {
            if(!add_to_tree(idx, parts, root.children[i])) {
                add_to_tree(idx+1, parts, root.children[i]);
                found = false;
                break;
            }
        }
...
function treeByColumn(dataView,col,seperator)
{
    ...
    for (var r = 0; r < ldata.length; ++r) {
        ...
        for (var partidx = 0; partidx < parts.length; ++parts) {
            var 
            for(treeidx = 0; treeidx < treedata.length; ++treedata) {
