...
init(_Transport, Req, _Args) ->
    ...
    {SP, IdpMeta} = initialize(binary_to_list(HostUrl) ++ "/", binary_to_list(Url)),
    {Path, Req2} = cowboy_req:path(Req),
    {Method, Req3} = cowboy_req:method(Req2),
    process_req(Method, Path, Req3, #state{sp = SP, idp = IdpMeta}).

process_req(<<"POST">>, <<"/saml">>, Req, S = #state{sp = SP}) ->
