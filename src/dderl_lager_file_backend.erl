...
-module(dderl_lager_file_backend).
...
init([{FileName, LogLevel, Size, Period, MoveTo}, {Formatter, FormatterConfig}]) when is_list(FileName), is_atom(LogLevel), is_integer(Period), is_list(MoveTo), is_atom(Formatter) ->
    %% coming from MPRO
    init([{file, FileName}, {level, LogLevel}, {size, Size}, {period, Period}, {move_to_folder, MoveTo}, {formatter, Formatter}, {formatter_config, FormatterConfig}]);
...
init(LogFileConfig) when is_list(LogFileConfig) ->
    case validate_logfile_proplist(LogFileConfig) of
        ...
        Config ->
            ...
            [Name, Level, Date, Size, Count, SyncInterval, SyncSize, SyncOn, CheckInterval, Formatter, FormatterConfig, Period, MoveTo] =
              [proplists:get_value(Key, Config) || Key <- [file, level, date, size, count, sync_interval, sync_size, sync_on, check_interval, formatter, formatter_config, period, move_to_folder]],
            ...
            State0 = #state{name=Name, level=Level, size=Size, date=Date, period=Period, count=Count, formatter=Formatter,
                ...
                check_interval=CheckInterval, move_to_folder=MoveTo},
...
handle_event({log, Message},
    ...
    case lager_util:is_loggable(Message,L,{dderl_lager_file_backend, Name}) andalso
...
rotate_logfile(File, _, MoveTo, State) ->
    {{Y,M,D},{H,MM,S}} = calendar:now_to_datetime(erlang:now()),
    NewFile = filename:join([
                filename:dirname(File), MoveTo,
                lists:flatten([
    ...
    case filelib:ensure_dir(NewFile) of
        ok ->
            _ = file:rename(File, NewFile),
            file:close(State#state.fd),
            case lager_util:open_logfile(File, {State#state.sync_size, State#state.sync_interval}) of
                {ok, {FD, Inode, _}} ->
                    NewState = State#state{fd = FD, inode = Inode};
                {error, Reason} ->
                    ?INT_LOG(error, "Failed to open log file ~s with error ~s", [File, file:format_error(Reason)]),
                    NewState = State#state{flap = true}
            end;
        _ ->
            ?INT_LOG(error, "Path cannot be created ~p", [NewFile]),
            NewState = State#state{flap = true}
    end,
    NewState.

%% @private
handle_info({rotate, File}, #state{name=File,count=Count,date=Date,period=Period,move_to_folder=MoveTo} = State) ->
    NewState = rotate_logfile(File, Count, MoveTo, State),
...
write(#state{name=Name, fd=FD, inode=Inode, flap=Flap, size=RotSize,
        count=Count, move_to_folder=MoveTo} = State, Timestamp, Level, Msg) ->
        ...
        true ->
            ...
            case lager_util:ensure_logfile(Name, FD, Inode, {State#state.sync_size, State#state.sync_interval}) of
                {ok, {_, _, Size}} when RotSize /= 0, Size > RotSize ->
                    ...
                    NewState = rotate_logfile(Name, Count, MoveTo, State),
...
validate_logfile_proplist([{move_to_folder, MoveTo}|Tail], Acc) ->
    case MoveTo of
        Val when is_list(MoveTo) ->
            validate_logfile_proplist(Tail, [{move_to_folder, Val}|Acc]);
        _ ->
            throw({bad_config, "Invalid Name for MoveToFolder", MoveTo})
    end;
    ...
    {foreach,
        ...
        [
            {"under normal circumstances, file should be opened",
                fun() ->
                        gen_event:add_handler(lager_event, dderl_lager_file_backend, [{"test.log", info}, {lager_default_formatter}]),
            ...
            {"file can't be opened on startup triggers an error message",
                fun() ->
                        ...
                        gen_event:add_handler(lager_event, dderl_lager_file_backend, {"test.log", info, 10*1024*1024, "$D0", 5}),
            ...
            {"file that becomes unavailable at runtime should trigger an error message",
                fun() ->
                        gen_event:add_handler(lager_event, dderl_lager_file_backend, [{file, "test.log"}, {level, info}, {check_interval, 0}]),
            ...
            {"unavailable files that are fixed at runtime should start having log messages written",
                fun() ->
                        ...
                        file:write_file_info("test.log", FInfo#file_info{mode = 0}),
                        gen_event:add_handler(lager_event, dderl_lager_file_backend, [{file, "test.log"},{check_interval, 0}]),
                        ?assertEqual(1, lager_test_backend:count()),
                        {_Level, _Time, Message,_Metadata} = lager_test_backend:pop(),
                        ?assertEqual("Failed to open log file test.log with error permission denied", lists:flatten(Message)),
            ...
            {"internal size rotation should work",
                fun() ->
                        gen_event:add_handler(lager_event, dderl_lager_file_backend, [{file, "test.log"}, {level, info}, {check_interval, 0}, {size, 10}]),
                        lager:log(error, self(), "Test message1"),
                        lager:log(error, self(), "Test message1"),
                        ?assert(filelib:is_regular("test.log.0"))
                end
            ...
            {"internal time rotation should work",
                fun() ->
                        gen_event:add_handler(lager_event, dderl_lager_file_backend, [{file, "test.log"}, {level, info}, {check_interval, 1000}]),
                        lager:log(error, self(), "Test message1"),
                        lager:log(error, self(), "Test message1"),
            ...
            {"sync_on option should work",
                fun() ->
                        gen_event:add_handler(lager_event, dderl_lager_file_backend, [{file, "test.log"}, {level, info}, {sync_on, "=info"}, {check_interval, 5000}, {sync_interval, 5000}]),
                        lager:log(error, self(), "Test message1"),
                        lager:log(error, self(), "Test message1"),
            ...
            {"sync_on none option should work (also tests sync_interval)",
                fun() ->
                        gen_event:add_handler(lager_event, dderl_lager_file_backend, [{file, "test.log"}, {level, info}, {sync_on, "none"}, {check_interval, 5000}, {sync_interval, 1000}]),
                        lager:log(error, self(), "Test message1"),
                        lager:log(error, self(), "Test message1"),
                        ?assertEqual({ok, <<>>}, file:read_file("test.log")),
                        lager:log(info, self(), "Test message1"),
            ...
            {"sync_size option should work",
                fun() ->
                        gen_event:add_handler(lager_event, dderl_lager_file_backend, [{file, "test.log"}, {level, info}, {sync_on, "none"}, {check_interval, 5001}, {sync_size, 640}, {sync_interval, 5001}]),
            ...
            {"runtime level changes",
                fun() ->
                        gen_event:add_handler(lager_event, {dderl_lager_file_backend, "test.log"}, {"test.log", info}),
                        ?assertEqual(0, lager_test_backend:count()),
                        lager:log(info, self(), "Test message1"),
                        lager:log(error, self(), "Test message2"),
                        {ok, Bin} = file:read_file("test.log"),
                        ...
                        ?assertEqual(Lines, 2),
                        ?assertEqual(ok, lager:set_loglevel(dderl_lager_file_backend, "test.log", warning)),
            ...
            {"invalid runtime level changes",
                fun() ->
                        gen_event:add_handler(lager_event, dderl_lager_file_backend, [{"test.log", info, 10*1024*1024, "$D0", 5}, {lager_default_formatter}]),
                        gen_event:add_handler(lager_event, dderl_lager_file_backend, {"test3.log", info}),
                        ?assertEqual({error, bad_module}, lager:set_loglevel(dderl_lager_file_backend, "test.log", warning))
                end
            },
            {"tracing should work",
                fun() ->
                        gen_event:add_handler(lager_event, dderl_lager_file_backend,
                            {"test.log", critical}),
                        ...
                        lager_config:set(loglevel, {Level, [{[{module,
                                                ?MODULE}], ?DEBUG,
                                        {dderl_lager_file_backend, "test.log"}}]}),
                        lager:error("Test message"),
                        timer:sleep(1000),
                        {ok, Bin} = file:read_file("test.log"),
    ...
    {foreach,
        ...
        fun(_) ->
            ...
            [{"Should have two log files, the second prefixed with 2>",
                fun() ->
                       gen_event:add_handler(lager_event, dderl_lager_file_backend,[{"test.log", debug},{lager_default_formatter,["[",severity,"] ", message, "\n"]}]),
                       gen_event:add_handler(lager_event, dderl_lager_file_backend,[{"test2.log", debug},{lager_default_formatter,["2> [",severity,"] ", message, "\n"]}]),
                       lager:log(error, self(), "Test message"),
